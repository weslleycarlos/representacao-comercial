...
class Organizacao(Base):
    ...
    dt_atualizacao = Column(
        "DT_ATUALIZACAO", DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
    )

    # Relacionamentos
    usuarios = relationship(
        "Usuario", back_populates="organizacao", cascade="all, delete-orphan"
    )
    empresas = relationship(
        "Empresa", back_populates="organizacao", cascade="all, delete-orphan"
    )
    clientes = relationship(
        "Cliente", back_populates="organizacao", cascade="all, delete-orphan"
    )
    categorias_produto = relationship(
        "CategoriaProduto", back_populates="organizacao", cascade="all, delete-orphan"
    )
    formas_pagamento = relationship(
        "FormaPagamento", back_populates="organizacao", cascade="all, delete-orphan"
    )
    regras_comissao = relationship(
        "RegraComissao", back_populates="organizacao", cascade="all, delete-orphan"
    )
    logs_auditoria = relationship(
        "LogAuditoria", back_populates="organizacao", cascade="all, delete-orphan"
    )
...
class Usuario(Base):
    ...
    id_organizacao = Column(
        "ID_ORGANIZACAO",
        Integer,
        ForeignKey("TB_ORGANIZACOES.ID_ORGANIZACAO", ondelete="CASCADE"),
        nullable=True,
    )
    ...
    id_usuario_criador = Column(
        "ID_USUARIO_CRIADOR", Integer, ForeignKey("TB_USUARIOS.ID_USUARIO")
    )
    dt_criacao = Column("DT_CRIACAO", DateTime, default=datetime.utcnow)
    dt_atualizacao = Column(
        "DT_ATUALIZACAO", DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
    ...
    criador = relationship(
        "Usuario", remote_side=[id_usuario], backref="usuarios_criados"
    )
    empresas_vinculadas = relationship(
        "UsuarioEmpresa", back_populates="usuario", cascade="all, delete-orphan"
    )
    pedidos = relationship(
        "Pedido", back_populates="vendedor", foreign_keys="Pedido.id_usuario"
    )
    comissoes_pedido = relationship(
        "ComissaoPedido",
        back_populates="vendedor",
        foreign_keys="ComissaoPedido.id_usuario",
    )
    logs_auditoria = relationship(
        "LogAuditoria", back_populates="usuario", foreign_keys="LogAuditoria.id_usuario"
    )
    regras_comissao = relationship(
        "RegraComissao",
        back_populates="usuario",
        foreign_keys="RegraComissao.id_usuario",
    )

    def set_password(self, password):
        self.ds_senha_hash = bcrypt.hashpw(
    ...
    def check_password(self, password):
        ...
        return bcrypt.checkpw(
            password.encode("utf-8"), self.ds_senha_hash.encode("utf-8")
        )
...
class Empresa(Base):
    ...
    __table_args__ = (
        UniqueConstraint("ID_ORGANIZACAO", "NR_CNPJ", name="UK_EMPRESAS_CNPJ_ORG"),
    )

    organizacao = relationship("Organizacao", back_populates="empresas")
    usuarios_vinculados = relationship(
        "UsuarioEmpresa", back_populates="empresa", cascade="all, delete-orphan"
    )
    produtos = relationship(
        "Produto", back_populates="empresa", cascade="all, delete-orphan"
    )
    regras_comissao = relationship(
        "RegraComissao", back_populates="empresa", cascade="all, delete-orphan"
    )
    pedidos = relationship(
        "Pedido", back_populates="empresa", cascade="all, delete-orphan"
    )
    catalogos = relationship(
        "Catalogo", back_populates="empresa", cascade="all, delete-orphan"
    )
...
class UsuarioEmpresa(Base):
    ...
    id_usuario = Column(
        "ID_USUARIO",
        Integer,
        ForeignKey("TB_USUARIOS.ID_USUARIO", ondelete="CASCADE"),
        primary_key=True,
    )
    id_empresa = Column(
        "ID_EMPRESA",
...
class Cliente(Base):
    ...
    __table_args__ = (
        UniqueConstraint("ID_ORGANIZACAO", "NR_CNPJ", name="UK_CLIENTES_CNPJ_ORG"),
    )
    ...
    contatos = relationship(
        "Contato", back_populates="cliente", cascade="all, delete-orphan"
    )
    pedidos = relationship(
...
class Endereco(Base):
    ...
    id_cliente = Column(
        "ID_CLIENTE",
        Integer,
        ForeignKey("TB_CLIENTES.ID_CLIENTE", ondelete="CASCADE"),
        nullable=False,
    )
...
class CategoriaProduto(Base):
    ...
    id_categoria_pai = Column(
        "ID_CATEGORIA_PAI", Integer, ForeignKey("TB_CATEGORIAS_PRODUTOS.ID_CATEGORIA")
    )
    ...
    parent = relationship(
        "CategoriaProduto", remote_side=[id_categoria], backref="children"
    )
...
class Produto(Base):
    ...
    id_categoria = Column(
        "ID_CATEGORIA", Integer, ForeignKey("TB_CATEGORIAS_PRODUTOS.ID_CATEGORIA")
    )
    ...
    __table_args__ = (
        UniqueConstraint("ID_EMPRESA", "CD_PRODUTO", name="UK_PRODUTOS_CODIGO_EMPRESA"),
    )
    ...
    listas_de_preco = relationship(
        "ItemCatalogo", back_populates="produto", cascade="all, delete-orphan"
    )
...
class VariacaoProduto(Base):
    ...
    id_produto = Column(
        "ID_PRODUTO",
        Integer,
        ForeignKey("TB_PRODUTOS.ID_PRODUTO", ondelete="CASCADE"),
        nullable=False,
    )
...
class FormaPagamento(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "ID_ORGANIZACAO", "NO_FORMA_PAGAMENTO", name="UK_FORMAS_PAGAMENTO_NOME"
        ),
    )
...
class RegraComissao(Base):
    ...
    usuario = relationship(
        "Usuario", back_populates="regras_comissao", foreign_keys=[id_usuario]
    )
...
class Pedido(Base):
    ...
    id_usuario = Column(
        "ID_USUARIO", Integer, ForeignKey("TB_USUARIOS.ID_USUARIO"), nullable=False
    )  # Vendedor
    id_empresa = Column(
        "ID_EMPRESA", Integer, ForeignKey("TB_EMPRESAS.ID_EMPRESA"), nullable=False
    )
    id_cliente = Column(
        "ID_CLIENTE", Integer, ForeignKey("TB_CLIENTES.ID_CLIENTE"), nullable=False
    )
    id_endereco_entrega = Column(
        "ID_ENDERECO_ENTREGA", Integer, ForeignKey("TB_ENDERECOS.ID_ENDERECO")
    )
    id_endereco_cobranca = Column(
        "ID_ENDERECO_COBRANCA", Integer, ForeignKey("TB_ENDERECOS.ID_ENDERECO")
    )
    id_forma_pagamento = Column(
        "ID_FORMA_PAGAMENTO",
        Integer,
        ForeignKey("TB_FORMAS_PAGAMENTO.ID_FORMA_PAGAMENTO"),
    )
    ...
    vendedor = relationship(
        "Usuario", back_populates="pedidos", foreign_keys=[id_usuario]
    )
    ...
    comissoes = relationship(
        "ComissaoPedido", back_populates="pedido", cascade="all, delete-orphan"
    )
...
class ItemPedido(Base):
    ...
    id_pedido = Column(
        "ID_PEDIDO",
        Integer,
        ForeignKey("TB_PEDIDOS.ID_PEDIDO", ondelete="CASCADE"),
        nullable=False,
    )
    id_produto = Column(
        "ID_PRODUTO", Integer, ForeignKey("TB_PRODUTOS.ID_PRODUTO"), nullable=False
    )
    id_variacao = Column(
        "ID_VARIACAO",
        Integer,
        ForeignKey("TB_VARIACOES_PRODUTOS.ID_VARIACAO"),
        nullable=True,
    )
...
class ComissaoPedido(Base):
    ...
    vendedor = relationship(
        "Usuario", back_populates="comissoes_pedido", foreign_keys=[id_usuario]
    )
...
class LogAuditoria(Base):
    ...
    id_usuario = Column(
        "ID_USUARIO", Integer, ForeignKey("TB_USUARIOS.ID_USUARIO", ondelete="SET NULL")
    )
    ...
    usuario = relationship(
        "Usuario", back_populates="logs_auditoria", foreign_keys=[id_usuario]
    )
...
class Catalogo(Base):
    ...
    __table_args__ = (
        UniqueConstraint("ID_EMPRESA", "NO_CATALOGO", name="UK_CATALOGO_NOME_EMPRESA"),
    )

    # Relacionamentos
    empresa = relationship("Empresa", back_populates="catalogos")
    itens_catalogo = relationship(
        "ItemCatalogo", back_populates="catalogo", cascade="all, delete-orphan"
    )
...
class ItemCatalogo(Base):
    ...
    id_catalogo = Column(
        "ID_CATALOGO",
        Integer,
        ForeignKey("TB_CATALOGOS.ID_CATALOGO", ondelete="CASCADE"),
        nullable=False,
    )
    id_produto = Column(
        "ID_PRODUTO",
    ...
    __table_args__ = (
        UniqueConstraint("ID_CATALOGO", "ID_PRODUTO", name="UK_CATALOGO_PRODUTO"),
    )
...
class RecuperacaoSenha(Base):
    __tablename__ = "TB_RECUPERACAO_SENHA"

    id_recuperacao = Column("ID_RECUPERACAO", Integer, primary_key=True)
    id_usuario = Column(
        "ID_USUARIO",
        Integer,
        ForeignKey("TB_USUARIOS.ID_USUARIO", ondelete="CASCADE"),
    ...
    ds_token = Column("DS_TOKEN", String(255), unique=True, nullable=False)
    dt_expiracao = Column("DT_EXPIRACAO", DateTime, nullable=False)
    dt_criacao = Column("DT_CRIACAO", DateTime, default=datetime.utcnow)

    usuario = relationship("Usuario", backref="tokens_recuperacao")
