...
from src.schemas import (
    AdminOrganizacaoCreate,
    AdminOrganizacaoUpdate,
    OrganizacaoSchema,
    OrganizacaoDetalhadaSchema,
    UsuarioSchema,
)
...
@admin_orgs_router.post(
    "/", response_model=OrganizacaoSchema, status_code=status.HTTP_201_CREATED
)
def create_organizacao_e_gestor(
    org_in: AdminOrganizacaoCreate, db: Session = Depends(get_db)
):
    """
    ...
    if (
        ...
        .filter(models.Usuario.ds_email == org_in.gestor.ds_email)
        .first()
    ):
    ...
    if (
        org_in.nr_cnpj
        and db.query(models.Organizacao)
        .filter(models.Organizacao.nr_cnpj == org_in.nr_cnpj)
        .first()
    ):
        raise HTTPException(
            status_code=409, detail="CNPJ da organização já está em uso."
        )
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Erro ao criar organização: {str(e)}"
        )
...
def get_todas_organizacoes(
    ...
    orgs = (
        db.query(models.Organizacao)
        .order_by(models.Organizacao.no_organizacao)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@admin_orgs_router.get("/{id_organizacao}", response_model=OrganizacaoDetalhadaSchema)
def get_organizacao_por_id(id_organizacao: int, db: Session = Depends(get_db)):
    """(Super Admin) Retorna detalhes de uma organização, incluindo seus gestores"""
    db_org = db.get(models.Organizacao, id_organizacao)
    if not db_org:
        raise HTTPException(status_code=404, detail="Organização não encontrada.")

    # Busca os gestores dessa organização
    gestores = (
        db.query(models.Usuario)
        .filter(
    ...
    # Monta a resposta
    org_detalhada = OrganizacaoDetalhadaSchema.model_validate(
        db_org, from_attributes=True
    )
    org_detalhada.gestores = [
        UsuarioSchema.model_validate(g, from_attributes=True) for g in gestores
    ]

    return org_detalhada


@admin_orgs_router.put("/{id_organizacao}", response_model=OrganizacaoSchema)
def update_organizacao(
    id_organizacao: int, org_in: AdminOrganizacaoUpdate, db: Session = Depends(get_db)
):
    ...
    if "nr_cnpj" in update_data and update_data["nr_cnpj"] != db_org.nr_cnpj:
        if (
            ...
            raise HTTPException(
                status_code=409, detail="CNPJ já está em uso por outra organização."
            )
